...
import { BarChart3, FileText, Home, LifeBuoy, type LucideIcon, Menu, Settings, X } from 'lucide-react';
...
export const SideBar: React.FC<DashboardSidebarProps> = ({ navigation = defaultNavigation }) => {
    ...
    if (item.isSection) {
      return (
        <div className="mt-4 px-3 py-2 first:mt-0">
          <h3 className="text-xs font-semibold tracking-wider text-gray-400 uppercase">{item.sectionTitle}</h3>
        </div>
      );
    }
      ...
      <div className="mb-1">
        <div
          className={cn(
            ...
            isActive ? 'bg-white/10 text-white' : 'text-gray-300 opacity-60 hover:opacity-100',
          )}>
          <Icon
            className={cn(
          ...
          <div className="border-t border-white/10 p-4">
            <Link
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">Guide and references</div>
              </div>
            </Link>
          </div>
        </div>
